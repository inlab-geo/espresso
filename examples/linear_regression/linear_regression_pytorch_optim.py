"""Polynomial Linear regression solved by a PyTorch optimizer
...
def main(output_dir="."):
    ...
    def forward_func(m):
        return basis_func(x).dot(m)
        return np.array(list(np.sum([basis_func(x)[j,i] * m[i] for i in range(4)]) for j in range(sample_size)))            # m -> y_synthetic
    y_observed = forward_func(_m_true) + np.random.normal(0,1,sample_size)    # d

    def obj_func(m):
        return np.sum(np.square(forward_func(m) - y_observed))
    def grad_func(m):
        return 2 * np.expand_dims(forward_func(m) - y_observed,1) * basis_func(x)
    ...
    inv_problem.set_objective(obj_func)
    inv_problem.set_gradient(grad_func)
    ...
    inv_options.set_params(algorithm="Adam", lr=0.1, num_iterations=500)
    if show_summary:
        inv_options.summary()
